...
from helper.RedisHelper import RedisDb
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def prepare(self):
        ...
        if method.upper() in ['POST','PUT']:
            ...
            try:
                ...
                log.err("JSON Error:%s , error:%s", self.request.arguments, str(e))
                self.write(ERR_INVALID.message)
                self.finish()
                return
    ...
    def set_user_cookie_record(self, cookie, userid):
        USER_COOKIE_TIME_OUT = 3600 * 24 * 30
        RedisDb.setex(cookie, userid, USER_COOKIE_TIME_OUT)
    ...
    def write(self, msg):
        ...
        # log.info("%s self.needCrypto",self.needCrypto)
        if self.needCrypto:
