...
from helper.DbHelper import Db, MongoDb
from helper.RedisHelper import RedisDBConfig
from lib import log, wx
from model.MeFile import MeFileConfig
from api.BaseHandler import BaseConfig
import sys, getopt
from model.SmsCode import SmsCodeConfig
...
class Application(tornado.web.Application):
    def __init__(self):
        ...
        self.handlers = [('/1.0/class/(\\w+)/(\\w+)', 'api.ClassHandler.ClassHandler'),
                         ('/1.0/class/(\\w+)', 'api.ClassHandler.ClassHandler'),
                         ('/1.0/classV2/(\\w+)/(\\w+)', 'api.ClassV2Handler.ClassV2Handler'),
                         ('/1.0/classV2/(\\w+)', 'api.ClassV2Handler.ClassV2Handler'),
                         ('/1.0/query/', 'api.QueryCountHandler.QueryCountHandler'),
                         ('/1.0/query/(\\w+)', 'api.QueryCountHandler.QueryCountHandler'),
                         ...
                         ('/1.0/file/download/(\\w+)', 'api.FileDownloadHandler.FileDownloadHandler'),
                         ('/sms/(.+)', 'api.SMSHandler.SMSHandler'),
                         ('/captcha/(.+)', 'api.CaptchaHandler.CaptchaHandler'),
                         ('/1.0/follow/(\\w+)/(\\w+)/(\\d+)', 'api.FollowerHandler.FollowerHandler'),
                         ('/1.0/follow/(\\w+)', 'api.FollowerHandler.FollowerHandler'),
                         ('/wx/(\\w+)', 'api.WxHandler.WxHandler'),
                         ('/1.0/recommend/(\\w+)/(\\w+)/(\\w+)', 'api.FollowerHandler.FollowerHandler'),
                         ('/1.0/follow/(\\w+)', 'api.FollowerHandler.FollowerHandler')]
        ...
        # projectClassHelper.index([("classname",1)], unique=True)
    ...
    def initOSS(self):
        ...
        MeFileConfig.bucket = oss2.Bucket(MeFileConfig.auth, "http://" + MeFileConfig.endpoint, MeFileConfig.bucket_name)
    ...
    def initDb(self):
        ...
        if self.config.has_option('mongodb', 'MONGO_ADDR1'):
            ...
            MeFileConfig.bucket = oss2.Bucket(MeFileConfig.auth, 'http://' + MeFileConfig.endpoint, MeFileConfig.bucket_name, is_cname=True)
        else:
        ...
        # 单工程模式
        if self.config.get('global', 'project_type') == 0:
